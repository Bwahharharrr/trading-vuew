...
function layout_cnv_cached(self, cache) {
	const $p = self.$props;
	const lay = $p.layout;
	const sub = $p.data;
	const n = sub.length;
...
*/
function drawCandles(ctx, candles, overlay) {
	const n = candles.length;
	if (!n) return;
	const wickGroups = /* @__PURE__ */ new Map();
	...
	for (const [color, rects] of fillGroups) {
		ctx.fillStyle = color;
		for (let i = 0, m = rects.length; i < m; i += 4) ctx.fillRect(rects[i], rects[i + 1], rects[i + 2], rects[i + 3]);
	}
...
*/
function drawVolbars(ctx, vols, overlay, layoutHeight) {
	const n = vols.length;
	if (!n) return;
	...
	for (let i = 0; i < n; i++) {
		...
		const color = data.green ? style.colorVolUp || "#23a77642" : style.colorVolDw || "#e5415042";
		...
		g.push(Math.floor(data.x1), Math.floor(layoutHeight - h - .5), Math.floor(data.x2 - data.x1), h + 1);
	}
	...
	methods: {
		...
		draw(ctx) {
			...
			if (isMainChart) cnv = {
				...
				if (!this._cnvCache) this._cnvCache = {
					key: "",
					val: null
				};
				cnv = layout_cnv_cached(this, this._cnvCache);
			}
			if (this.show_volume) drawVolbars(ctx, cnv.volume, this, this.$props.layout.height);
...
var _sfc_main$17 = {
	...
	methods: {
		...
		draw(ctx) {
			...
			if (this.marker_rule) {
				this.draw_symbol_markers(ctx, layout, data, r);
				return;
			}
			ctx.lineWidth = this.line_width;
	...
	computed: {
		...
		marker_rule() {
			return this.sett.corkyMarkerRule || null;
		},
		glyph_font() {
			return `${Math.max(10, Math.round(this.marker_size * 2))}px ` + this.$props.font.split("px").pop();
		},
		show_label() {
			return this.sett.showLabel !== false;
		},
...
function _sfc_render$9(_ctx, _cache, $props, $setup, $data, $options) {
	...
	return $props.common ? (openBlock(), createElementBlock("div", {
		...
		$props.grid_id === 0 && $options.show_volume_row && !$options.volume_detached ? (openBlock(), createElementBlock("div", _hoisted_7, [
			...
			}, [!$options.volume_detached ? (openBlock(), createElementBlock("svg", _hoisted_9, [..._cache[10] || (_cache[10] = [createElementVNode("path", {
				fill: "currentColor",
				d: "M7.41 8.59L12 13.17l4.59-4.58L18 10l-6 6-6-6z"
			}, null, -1)])])) : (openBlock(), createElementBlock("svg", _hoisted_10, [..._cache[11] || (_cache[11] = [createElementVNode("path", {
				fill: "currentColor",
...
var Botbar = class {
	...
	measureTextCached(text) {
		...
		measureTextCache$1.set(key, result.width);
		return result.width;
	}
	async listeners() {
		const { Hammer, Hamster } = await loadGestures();
...
function GridMaker(id, params, master_grid = null) {
	...
	function calc_sidebar() {
		...
		self.sb = measureTextCached(ctx, "0".repeat(utils_default.maxInArray(lens)) + "    ");
...
function querySearch(data, query, tuple) {
	...
	const match = (x) => x.id === query || x.id && x.id.includes(path) || x.name === query || x.name && x.name.includes(path) || (x.settings || {}).$uuid != null && query.includes(x.settings.$uuid);
	if (field) return sideData.filter(match).map((x) => ({
		p: x,
		i: field,
		v: x[field]
	}));
...
function mergeTs(obj, data) {
	...
	} else obj.p[obj.i] = markRaw(combine(obj.v, [], data));
	return obj.v;
}
//#endregion
